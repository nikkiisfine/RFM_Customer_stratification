...
This README was automatically generated by [Claude](https://claude.ai) (claude-sonnet-4-6) via the Claude Code CLI. Claude's contributions include:

- **Data source lookup**: Identified the origin of `PYTHON-RFM实战数据.xlsx` as a teaching dataset from the *数据不吹牛* Alibaba Cloud tutorial series, and retrieved the canonical source URLs.
- **README generation**: Read and parsed the full Jupyter notebook (`RFM模型实战.ipynb`), extracted methodology details, scoring rules, segmentation logic, and result tables, then structured and wrote this document in English.
